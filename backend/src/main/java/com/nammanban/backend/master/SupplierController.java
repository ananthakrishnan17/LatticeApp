...
import static com.nammanban.backend.master.MasterDtos.SupplierRecord;
import static com.nammanban.backend.master.MasterDtos.SupplierUpsertRequest;
...
public class SupplierController {
    ...
    @GetMapping
    public List<SupplierRecord> list() {
        UUID tenantId = TenantContext.tenantId();
        return jdbc.queryForList("""
            SELECT server_id, client_record_id, name, phone, address, gst_number,
                   outstanding_balance, version, updated_at
            FROM app_core.suppliers
            WHERE tenant_id = ? AND deleted_at IS NULL
            ORDER BY name ASC
            """, tenantId)
                .stream()
                .map(r -> new SupplierRecord(
    ...
    @PostMapping("/upsert")
    @Transactional
    public Map<String, Object> upsert(@Valid @RequestBody SupplierUpsertRequest request) {
        UUID tenantId = TenantContext.tenantId();
        ...
        jdbc.update("""
            INSERT INTO app_core.suppliers(
              tenant_id, client_record_id, device_id, name, phone, address, gst_number,
              outstanding_balance, version, updated_at, deleted_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ...
            deviceId,
            request.name().trim(),
            toNullable(request.phone()),
            toNullable(request.address()),
            toNullable(request.gstNumber()),
