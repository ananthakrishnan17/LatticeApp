...
class _AllBillsPageState extends State<AllBillsPage> {
  ...
  List<Bill> _bills = [];
  Set<int> _modifiedBillIds = {};
  Map<int, String> _modificationNotes = {};
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final modifiedMeta =
          await _loadModifiedMeta(from: _fromDate, to: _toDate);
      if (mounted) {
        setState(() {
          _bills = data;
          _modifiedBillIds = modifiedMeta.$1;
          _modificationNotes = modifiedMeta.$2;
        });
      }
    } catch (e) {
  ...
  Future<(Set<int>, Map<int, String>)> _loadModifiedMeta({
    required DateTime from,
    required DateTime to,
  }) async {
    // Bill modification history is stored only in local SQLite.
    // In online mode SQLite is blocked, so skip the lookup and return empty.
    if (await DataAccessModeService.instance.resolveMode() ==
    ...
    try {
      final rows = await db.rawQuery('''
        SELECT h.bill_id, h.modification_note
        FROM bill_modification_history h
        INNER JOIN (
          SELECT bill_id, MAX(modified_at) as latest_modified_at
          FROM bill_modification_history
          WHERE modified_at BETWEEN ? AND ?
          GROUP BY bill_id
        ) latest ON latest.bill_id = h.bill_id
          AND latest.latest_modified_at = h.modified_at
      ''', [from.toIso8601String(), to.toIso8601String()]);
      final ids = <int>{};
      final notes = <int, String>{};
      for (final row in rows) {
        final id = row['bill_id'] as int?;
        if (id == null) continue;
        ids.add(id);
        final note = (row['modification_note'] as String?)?.trim();
      ...
      debugPrint(
          '[AllBillsPage] bill_modification_history lookup failed, falling back to bills.is_modified: $e');
      final rows = await db.query(
        'bills',
        columns: ['id', 'modification_note'],
        where: 'is_modified = 1 AND created_at BETWEEN ? AND ?',
  ...
  Widget _buildBillList() {
    return ListView.separated(
      ...
      itemBuilder: (_, i) => _BillCard(
        bill: _bills[i],
        isModified: _modifiedBillIds.contains(_bills[i].id),
        modificationNote: _modificationNotes[_bills[i].id],
