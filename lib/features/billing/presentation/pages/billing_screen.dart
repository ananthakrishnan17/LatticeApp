...
import '../../../cash_session/presentation/widgets/cash_denomination_dialog.dart';
...
class _BillingScreenState extends State<BillingScreen> {
  ...
  Future<void> _tryAutoPrint(Bill bill) async {
    try {
      ...
      debugPrint('[Billing] auto-print failed: $e\n$st');
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: const Text('Bill saved. Auto-print failed — please reprint manually.'),
          backgroundColor: AppTheme.warning,
          behavior: SnackBarBehavior.floating,
          duration: const Duration(seconds: 3),
          margin: EdgeInsets.only(bottom: 24.h, left: 16.w, right: 16.w),
        ...
        child: Scaffold(
          ...
          body: LayoutBuilder(
            builder: (context, constraints) {
              ...
              final productGridCount = responsive.gridCount(mobile: 3, tablet: 4, desktop: 6);
                  ...
                  children: [
                    ...
                    Expanded(
                      child: isTabletLayout
                          ? _buildTabletBillingBody(productGridCount)
                          : (_showCart
                              ? _buildCartView()
                              : _buildProductView(crossAxisCount: productGridCount)),
                    ),
                  ],
                ),
              );
  ...
  Widget _buildTabletBillingBody(int productGridCount) {
    final responsive = ResponsiveHelper.of(context);
    return Row(
      children: [
        Flexible(
          flex: 6,
          child: _buildProductView(crossAxisCount: productGridCount, showBottomBar: false),
        ),
          ...
          child: Column(
            children: [
              Padding(
                ...
                child: Row(
                  children: [
                    Text('Cart', style: AppTheme.heading3),
                    const Spacer(),
                    TextButton.icon(
                      onPressed: () => context.read<BillingBloc>().add(ClearCart()),
  ...
  Widget _buildTopBar() {
    return BlocBuilder<BillingBloc, CartState>(
      builder: (context, state) {
        ...
        final AppUser? currentUser = context.read<UserBloc>().currentUser;
        return Container(
          color: Colors.white,
          ...
          child: Row(
            children: [
              ...
              if (currentUser != null && !currentUser.isAdmin) ...[
                SizedBox(width: 8.w),
                GestureDetector(
                  onTap: () => _showCloseShiftDialog(currentUser),
                  child: Container(
                    width: 44.w,
                    height: 44.h,
                    decoration: BoxDecoration(
                    ...
                    child: Icon(Icons.nightlight_round,
                        color: AppTheme.secondary, size: 20.sp),
                  ),
                ),
              ],
              // ── Held Bills Badge ────────────────────────────────────────────
  ...
  // ── Product Grid ──────────────────────────────────────────────────────────
  Widget _buildProductView({int crossAxisCount = 3, bool showBottomBar = true}) {
    return BlocBuilder<BillingBloc, CartState>(
  ...
  Future<void> _showCloseShiftDialog(AppUser user) async {
    final active = await _cashSessionRepository.getActiveSession(user.username);
    if (active == null) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
        content: Text('No active shift found.'),
        backgroundColor: AppTheme.warning,
      ));
      return;
    }
    ...
    final closed = await _cashSessionRepository.closeSession(
      cashierUsername: user.username,
      closingAmount: result.total,
      closingDenominationCounts: result.counts,
    ...
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      content: Text('$message Difference: ${CurrencyFormatter.format(diff)}'),
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () {
                ...
                final sampleQty = formatter.format(1.5);
                double? qty;
                try {
                  qty = formatter.parse(raw).toDouble();
                } catch (e) {
                  debugPrint('[Billing] locale parse failed for quantity "$raw": $e');
                  qty = double.tryParse(raw.replaceAll(',', '.'));
                }
                if (qty == null || qty <= 0) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content: Text(
                        'Enter a valid quantity greater than 0 (e.g., $sampleQty)',
                      ),
                    ),
                  );
                  return;
  ...
  Future<void> _showItemDiscountDialog(CartItem item, CartState cart) async {
    final controller = TextEditingController(
      text: item.itemDiscountValue > 0
          ? item.itemDiscountValue.toStringAsFixed(
              item.itemDiscountValue % 1 == 0 ? 0 : 2,
            )
          : '',
    );
    var discountType = item.itemDiscountType;
        ...
        builder: (ctx) => StatefulBuilder(
          builder: (ctx, setDialogState) => AlertDialog(
            ...
            content: Column(
              mainAxisSize: MainAxisSize.min,
              children: [
                DropdownButtonFormField<ItemDiscountType>(
                  value: discountType,
                  decoration: const InputDecoration(labelText: 'Discount Type'),
                  items: ItemDiscountType.values
                      .map(
                ...
                TextField(
                  ...
                  decoration: InputDecoration(
                    labelText: discountType == ItemDiscountType.percent
                        ? 'Discount %'
                        : 'Discount Amount',
                    hintText: discountType == ItemDiscountType.none ? '0' : null,
                    suffixText: discountType == ItemDiscountType.percent ? '%' : null,
                    prefixText:
                        discountType == ItemDiscountType.amount ? '₹ ' : null,
                  ),
                ),
                SizedBox(height: 8.h),
                Align(
                  ...
                  child: Text(
                    'Row total: ${CurrencyFormatter.format(item.totalFor(cart.billType))}',
                ...
                onPressed: () {
                  context.read<BillingBloc>().add(
                        UpdateCartItemDiscount(
                          productId: item.productId,
                          saleUomId: item.saleUomId,
                          discountType: ItemDiscountType.none,
                          discountValue: 0,
                        ),
                      );
                  Navigator.pop(ctx);
                ...
                onPressed: () {
                  ...
                  context.read<BillingBloc>().add(
                        UpdateCartItemDiscount(
                          ...
                          saleUomId: item.saleUomId,
                          discountType:
                              value <= 0 ? ItemDiscountType.none : discountType,
                          discountValue: value <= 0 ? 0 : value,
                        ),
                      );
                  Navigator.pop(ctx);
                },
  ...
  Future<void> _showCustomerPicker() async {
    ...
    if (customer is Customer) {
      billingBloc.add(SetCustomer(customer));
    } else if (customer == 'walk_in') {
      billingBloc.add(SetCustomer(null));
    }
  }
                    ...
                    return CartItemTile(
                      ...
                      onDecrease: () => context.read<BillingBloc>().add(
                          UpdateCartItemQty(
                              ...
                              quantity: item.quantity - 1,
                              saleUomId: item.saleUomId)),
                      onQuantityTap: () => _showQuantityInputDialog(item),
                      onDiscountTap: () => _showItemDiscountDialog(item, cart),
                      onRemove: () => context
                          .read<BillingBloc>()
  ...
  Future<void> _showPaymentSheet(BuildContext context, CartState cart) async {
    ...
    if (user != null && !user.isAdmin) {
      ...
      if (active == null) {
        if (!mounted) return;
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('Open shift is required before billing.'),
          backgroundColor: AppTheme.warning,
        ));
        return;
      }
    }
    final savedBill = await showModalBottomSheet<Bill>(
      context: context,
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
    ...
    if (!mounted || savedBill == null) return;
    await _onBillSaved(context, savedBill);
  }
            ...
            onRestore: (bill) {
              ...
              final cartItems = bill.items
                  .map((item) => CartItem(
                        ...
                        itemDiscountType:
                            ItemDiscountTypeExt.fromValue(item.itemDiscountType),
              ...
              context.read<BillingBloc>().add(RestoreHeldCartItems(
                    ...
                    appliedCoupon: bill.couponCode != null &&
                            bill.couponCode!.isNotEmpty &&
                            bill.couponDiscountAmount > 0
                        ? AppliedCoupon(
                            couponId: bill.couponId,
                            code: bill.couponCode!,
                            discountType: CouponType.flat,
                            discountValue: bill.couponDiscountAmount,
                            discountAmount: bill.couponDiscountAmount,
                          )
                        : null,
                  ));
