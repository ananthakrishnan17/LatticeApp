...
class HeldBillItem {
  ...
  const HeldBillItem({this.id, required this.heldBillId, required this.productId,
    ...
    this.discountAmount = 0.0, this.itemDiscountType = 'none',
  ...
  factory HeldBillItem.fromMap(Map<String, dynamic> m) => HeldBillItem(
      ...
      itemDiscountType: m['item_discount_type'] as String? ?? 'none',
      itemDiscountValue: (m['item_discount_value'] as num?)?.toDouble() ?? 0,
...
class HeldBill {
  ...
  const HeldBill({this.id, this.holdName, this.billType = 'retail',
    ...
    this.couponId, this.couponCode, this.couponDiscountAmount = 0,
  ...
  factory HeldBill.fromMap(Map<String, dynamic> m, List<HeldBillItem> items) => HeldBill(
      ...
      discountAmount: (m['discount_amount'] as num?)?.toDouble() ?? 0,
      couponId: m['coupon_id'] as int?,
      couponCode: m['coupon_code'] as String?,
      couponDiscountAmount: (m['coupon_discount_amount'] as num?)?.toDouble() ?? 0,
...
class HeldBillRepository {
  ...
  Future<int> holdBill(CartState cart, {String? holdName}) async {
    if (await _isOnlineMode()) {
      throw StateError('Held bills are unavailable in online API mode.');
    }
    ...
    return await db.transaction((txn) async {
      ...
      for (final item in cart.items) {
        await txn.insert('held_bill_items', {
          ...
          'unit': item.unit, 'unit_price': item.effectivePrice(cart.billType),
          'purchase_price': item.purchasePrice,
          'discount_amount': item.itemDiscountAmountFor(cart.billType),
          'item_discount_type': item.itemDiscountType.value,
