...
class _AddPurchasePageState extends State<AddPurchasePage> {
  ...
  Future<void> _savePurchase() async {
    if (_invoiceAmount > 0 && _invoiceDiff.abs() >= 0.01) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text('Cannot save: Invoice amount (₹${_invoiceAmount.toStringAsFixed(2)}) does not match Computed Total (₹${_total.toStringAsFixed(2)}).'),
        backgroundColor: AppTheme.danger,
    ...
    context.read<PurchaseBloc>().add(SavePurchaseEvent(
      ...
      invoiceAmount: _invoiceAmount > 0 ? _invoiceAmount : null,
          ...
          child: SingleChildScrollView(
            ...
            ElevatedButton(
              onPressed: () {
                ...
                final parsedCost = double.tryParse(_costCtrl.text);
                if (qty == null || qty <= 0 || parsedCost == null || parsedCost <= 0) {
                  ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                    content: Text('Enter valid positive quantity and unit cost'),
                  ));
                  return;
                }
                final cost = parsedCost;
