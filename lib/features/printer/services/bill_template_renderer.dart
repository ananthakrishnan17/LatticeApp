...
class BillTemplateRenderer {
  ...
  static Future<void> _printQuick58mm({
    ...
    if (bill.paymentMode == 'split' && bill.splitPaymentSummary != null) {
      printer.printCustom(_thermalText(bill.splitPaymentSummary!), 1, 1);
    }
    printer.printNewLine();
    printer.printCustom(div, 1, 1);
    ...
    for (final item in bill.items) {
      ...
      printer.printLeftRight(
        '  $qtyStr x ${_thermalShort(item.unitPrice)}',
        _thermalShort(item.totalPrice),
        1,
      );
      if (config.showGst && item.gstRate > 0 && item.gstAmount > 0) {
        printer.printLeftRight(
          '  GST ${item.gstRate.toStringAsFixed(0)}%',
          _thermalShort(item.gstAmount),
          1,
        );
      }
    }
    ...
    if (config.showDiscount && bill.discountAmount > 0) {
      printer.printLeftRight(
        'Subtotal:',
        _thermalAmount(bill.totalAmount + bill.discountAmount),
        1,
      );
      printer.printLeftRight(
        'Discount:',
        '-${_thermalAmount(bill.discountAmount)}',
        1,
      );
    }
    if (config.showGst && bill.gstTotal > 0) {
      printer.printLeftRight(
          'CGST:', _thermalAmount(bill.cgstTotal), 1);
      printer.printLeftRight(
          'SGST:', _thermalAmount(bill.sgstTotal), 1);
    }

    printer.printCustom(
      'TOTAL: ${_thermalAmount(bill.totalAmount)}',
      2,
      1,
    );
  ...
  static Future<void> _printPremium80mm({
    ...
    if (bill.paymentMode == 'split' && bill.splitPaymentSummary != null) {
      printer.printCustom('  ${_thermalText(bill.splitPaymentSummary!)}', 1, 0);
    }
    if (bill.customerName != null && bill.customerName!.isNotEmpty) {
      printer.printLeftRight('Customer   :', bill.customerName!, 1);
    }
    ...
    // Header
    printer.printLeftRight('Item Details', 'Total', 1);
    ...
    final subtotal = config.showDiscount && bill.discountAmount > 0
        ...
        : bill.totalAmount;
    printer.printLeftRight(
        'Subtotal:', _thermalAmount(subtotal), 1);
    if (config.showDiscount && bill.discountAmount > 0) {
      printer.printLeftRight(
          'Discount:', '-${_thermalAmount(bill.discountAmount)}', 1);
    }
    if (config.showGst && bill.gstTotal > 0) {
      printer.printLeftRight(
          'CGST:', _thermalAmount(bill.cgstTotal), 1);
      ...
      if (bill.igstTotal > 0) {
        printer.printLeftRight(
            'IGST:', _thermalAmount(bill.igstTotal), 1);
      }
      printer.printLeftRight(
          'GST Total:', _thermalAmount(bill.gstTotal), 1);
    }
    printer.printCustom(
      'GRAND TOTAL: ${_thermalAmount(bill.totalAmount)}',
      2,
      1,
    );
  ...
  static Future<void> _printGstInvoice({
    ...
    // Header
    printer.printLeftRight('Item (Taxable / CGST / SGST)', 'Total', 1);
    printer.printCustom(thin, 1, 1);
    ...
    for (final item in bill.items) {
      ...
      printer.printCustom(item.productName, 1, 0);
      printer.printLeftRight(
        '  $qtyStr x ${_thermalShort(item.unitPrice)} | Tax: ${_thermalShort(taxable)}',
        _thermalShort(item.totalPrice),
        1,
      );
      if (item.gstRate > 0) {
        printer.printLeftRight(
          '  ${item.gstRate.toStringAsFixed(0)}% (CGST: ${_thermalShort(halfGst)} SGST: ${_thermalShort(halfGst)})',
          '',
          1,
        );
      }
    ...
    printer.printLeftRight(
        'Taxable Amount:', _thermalAmount(taxable), 1);
    printer.printLeftRight(
        'CGST:', _thermalAmount(bill.cgstTotal), 1);
    printer.printLeftRight(
        'SGST:', _thermalAmount(bill.sgstTotal), 1);
    ...
    printer.printLeftRight(
        'Total Tax:', _thermalAmount(bill.gstTotal), 1);
    if (config.showDiscount && bill.discountAmount > 0) {
      printer.printLeftRight(
          'Discount:', '-${_thermalAmount(bill.discountAmount)}', 1);
    }
  ...
  static Future<void> _printRestaurantKot({
    ...
    for (final item in bill.items) {
      ...
      printer.printLeftRight(
        ...
        _thermalAmount(item.totalPrice),
        1,
      );
      printer.printNewLine();
    }
  ...
  static String _thermalAmount(double value) =>
      CurrencyFormatter.format(value).replaceAll('₹', 'Rs ');

  static String _thermalShort(double value) =>
      CurrencyFormatter.short(value).replaceAll('₹', 'Rs');

  static String _thermalText(String value) => value.replaceAll('₹', 'Rs ');
}
