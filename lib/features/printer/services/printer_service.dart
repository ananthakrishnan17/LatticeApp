...
class PrinterService {
  ...
  Future<bool> printBill(Bill bill) async {
    ...
    try {
      ...
      if (bill.paymentMode == 'split' && bill.splitPaymentSummary != null) {
        ...
        _printer.printCustom(_thermalText(bill.splitPaymentSummary!), 1, 1);
      } else {
      ...
      for (final item in bill.items) {
        ...
        final line =
            ...
            _col(_thermalShort(item.unitPrice),  7, rightAlign: true) +
            _col(_thermalShort(item.totalPrice), 7, rightAlign: true);
        ...
        if (item.gstRate > 0 && item.gstAmount > 0) {
          final gstLine =
              ...
              _col(_thermalShort(item.gstAmount), 14, rightAlign: true);
      ...
      if (bill.discountAmount > 0) {
        _printer.printLeftRight(
          'Subtotal:', _thermalAmount(bill.totalAmount + bill.discountAmount), 1);
        _printer.printLeftRight(
      ...
      _printer.printCustom(
        'TOTAL: ${_thermalAmount(bill.totalAmount)}', 2, 1);
  ...
  String _thermalAmount(double value) =>
      CurrencyFormatter.format(value).replaceAll('₹', 'Rs ');

  String _thermalShort(double value) =>
      CurrencyFormatter.short(value).replaceAll('₹', 'Rs');

  String _thermalText(String value) => value.replaceAll('₹', 'Rs ');
}
