  ...
  Future<void> _export() async {
    ...
    await PdfExportHelper.exportAndShare(
      ...
      rows: _rows.map((d) {
        ...
        final wholesaleUnit = d['wholesale_unit']?.toString() ??
            (d['unit']?.toString() ?? 'pcs');
        final retailUnit =
            d['retail_unit']?.toString() ?? (d['unit']?.toString() ?? 'pcs');
        final purchasedUnit = wsToRetail > 1.0 ? wholesaleUnit : retailUnit;
        ...
        final wholesaleSoldQty =
            (d['total_wholesale_sold_qty'] as num).toDouble();
        return [
          d['name']?.toString() ?? '',
          d['category_name']?.toString() ?? '',
          '${(d['total_purchased_qty'] as num).toStringAsFixed(2)} $purchasedUnit',
          CurrencyFormatter.format((d['total_purchase_value'] as num).toDouble()),
          '${wholesaleSoldQty.toStringAsFixed(2)} $wholesaleUnit',
          '${(d['total_retail_sold_qty'] as num).toStringAsFixed(2)} $retailUnit',
...
class _ProductStockCardState extends State<_ProductStockCard> {
  ...
  Widget build(BuildContext context) {
    ...
    final wholesaleUnit = row['wholesale_unit']?.toString() ??
        (row['unit']?.toString() ?? 'pcs');
    final retailUnit = row['retail_unit']?.toString() ??
        (row['unit']?.toString() ?? 'pcs');
    final purchasedUnit = wsToRetail > 1.0 ? wholesaleUnit : retailUnit;
              ...
              child: Column(
                children: [
                  _detailRow('📥', 'Purchased',
                      '${_fmtQty(purchasedQty)} $purchasedUnit'),
                  SizedBox(height: 4.h),
                  if (wsToRetail > 1.0) ...[
