...
class _StockLedgerPageState extends State<StockLedgerPage> {
  ...
  String? _error;
  String get _selectedUnit => _selectedProduct?['unit'] as String? ?? 'pcs';
  ...
  Future<void> _loadProducts() async {
    try {
      ...
      final rows = await db.rawQuery(
          'SELECT id, name, unit, stock_quantity FROM products WHERE is_active=1 ORDER BY name ASC');
  ...
  Future<void> _loadLedger() async {
    ...
    try {
      ...
      final productUnit = _selectedProduct!['unit'] as String? ?? 'pcs';
      final currentStockUser = UomConversionHelper.toUser(
        (_selectedProduct!['stock_quantity'] as num?)?.toDouble() ?? 0.0,
        productUnit,
      );
      ...
      final sales = await db.rawQuery('''
        SELECT b.created_at as date, 'Sale #' || b.bill_number as description,
               ...
               0.0 as qty_in,
               CASE
                 WHEN COALESCE(bi.sale_type, 'retail') = 'wholesale'
                      AND COALESCE(p.wholesale_to_retail_qty, 1.0) > 1.0
                   THEN bi.quantity * COALESCE(p.wholesale_to_retail_qty, 1.0)
                 ELSE bi.quantity * COALESCE(bi.conversion_qty, 1.0)
               END as qty_out
        FROM bill_items bi
      ...
      final purchases = await db.rawQuery('''
        SELECT p.created_at as date, 'Purchase #' || p.purchase_number as description,
               'purchase' as source,
               CASE
                 WHEN COALESCE(pr.wholesale_to_retail_qty, 1.0) > 1.0
                   THEN pi.quantity * COALESCE(pr.wholesale_to_retail_qty, 1.0)
                 ELSE pi.quantity
               END as qty_in,
               0.0 as qty_out
      ...
      final returns = await db.rawQuery('''
        SELECT sr.created_at as date, 'Return #' || sr.return_number as description,
               ...
               COALESCE(
                 sri.base_qty_restored,
                 CASE
                   WHEN COALESCE(sri.sale_type, 'retail') = 'wholesale'
                        AND COALESCE(sri.wholesale_to_retail_qty, 1.0) > 1.0
                     THEN sri.quantity * COALESCE(sri.wholesale_to_retail_qty, 1.0)
                   ELSE sri.quantity * COALESCE(sri.conversion_qty, 1.0)
                 END
               ) as qty_in,
               0.0 as qty_out
      ...
      final all = [...sales, ...purchases, ...returns, ...adjustments]
          .map((row) => {
                ...row,
                'sortDate': _parseDate((row['date'] as String?) ?? ''),
              })
          .toList();
      all.sort((a, b) =>
          (a['sortDate'] as DateTime).compareTo(b['sortDate'] as DateTime));

      final netMovement = all.fold<double>(0.0, (sum, r) {
        final qIn = (r['qty_in'] as num?)?.toDouble() ?? 0;
        final qOut = (r['qty_out'] as num?)?.toDouble() ?? 0;
        return sum + qIn - qOut;
      });
      final openingBalance = currentStockUser - netMovement;

      // Running balance (in user-facing unit)
      double balance = openingBalance;
      final withBalance = all.map((r) {
        final qIn = (r['qty_in'] as num?)?.toDouble() ?? 0;
        final qOut = (r['qty_out'] as num?)?.toDouble() ?? 0;
        balance += qIn - qOut;
        return {...r, 'qty_in': qIn, 'qty_out': qOut, 'balance': balance};
      }).toList();
          ...
          if (_selectedProduct != null)
            Container(
              ...
              child: Row(children: [
                ...
                Text(
                  _formatQty(UomConversionHelper.toUser(
                    (_selectedProduct!['stock_quantity'] as num?)?.toDouble() ?? 0.0,
                    _selectedUnit,
                  )) + ' $_selectedUnit',
          ...
          if (_entries.isNotEmpty)
            Container(
              ...
              child: Row(children: [
                ...
                _hdr('Stock In ($_selectedUnit)'),
                _hdr('Stock Out ($_selectedUnit)'),
                _hdr('Balance ($_selectedUnit)'),
              ]),
            ),
                                ...
                                itemBuilder: (_, i) {
                                  ...
                                  return Container(
                                    ...
                                    child: Row(children: [
                                      ...
                                      _cell(qIn > 0 ? '+${_formatQty(qIn)} $_selectedUnit' : '-',
                                          qIn > 0 ? AppTheme.accent : AppTheme.textSecondary),
                                      _cell(qOut > 0 ? '-${_formatQty(qOut)} $_selectedUnit' : '-',
                                          qOut > 0 ? AppTheme.danger : AppTheme.textSecondary),
                                      _cell('${_formatQty(bal)} $_selectedUnit', AppTheme.textPrimary, bold: true),
                                    ]),
                                  );
                                },
                              ),
  ...
  DateTime _parseDate(String raw) {
    try {
      ...
      debugPrint('[StockLedger] Invalid date "$raw": $e');
