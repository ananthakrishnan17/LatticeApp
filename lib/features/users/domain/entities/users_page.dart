...
class _UsersPageState extends State<UsersPage> {
  ...
  late Future<int> _maxCompaniesFuture;
  ...
  void initState() {
    ...
    _canAddMoreFuture = _canAddMoreUsers();
    _maxUsersFuture = SubscriptionService.instance.getMaxAllowedUsers();
    _maxCompaniesFuture = SubscriptionService.instance.getMaxAllowedCompanies();
                  ...
                  if (v == 'toggle') {
                    if (ctx.mounted) {
                      ctx.read<UserBloc>().add(
                        ToggleUserActive(user.username, !user.isActive),
                      );
                    }
                  }
                  if (v == 'delete' && !isCurrentUser) _confirmDelete(ctx, user);
                  if (v == 'change_pin') _showChangePinDialog(ctx, user);
                  if (v == 'force_close_shift' && !user.isAdmin) {
                    await _forceCloseCashierSession(ctx, user);
                  }
                },
                itemBuilder: (_) => [
                  if (!isCurrentUser)
                  ...
                  if (!user.isAdmin)
                    const PopupMenuItem(
                      value: 'force_close_shift',
                      child: Row(children: [Icon(Icons.lock_clock, size: 16), SizedBox(width: 8), Text('Force Close Shift')]),
                    ),
  ...
  Future<void> _forceCloseCashierSession(BuildContext context, AppUser user) async {
    final admin = context.read<UserBloc>().currentUser;
    if (admin == null || !admin.isAdmin) return;
    final active = await _cashSessionRepository.getActiveSession(user.username);
    if (active == null) {
      if (context.mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text('No active shift for this user.'),
          backgroundColor: AppTheme.warning,
        ));
      }
      return;
    }
    await _cashSessionRepository.closeSession(
      cashierUsername: user.username,
      closingAmount: active.expectedClosing,
      closedBy: admin.username,
      force: true,
      notes: 'Force closed by admin',
    );
    if (context.mounted) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text('Shift force-closed for ${user.username}.'),
        backgroundColor: AppTheme.warning,
      ));
    }
  }
                ...
                ElevatedButton(
                  onPressed: isSaving ? null : () async {
                    ...
                    if (!isEditing) {
                      ...
                      late final ({bool success, String? error}) result;
                      try {
                        await ctx.read<UserBloc>().repo.createUser(newUser);
                      ...
                      if (ctx.mounted) {
                        ctx.read<UserBloc>().add(LoadUsers());
                        Navigator.pop(ctx);
                      }
                    } else {
                      final user = AppUser(
                      ...
                      setSt(() { isSaving = true; saveError = null; });
                      final cloudUpdated = await ctx.read<UserBloc>().repo.updateUser(user);
                      if (!cloudUpdated) {
                        setSt(() { saveError = 'Failed to update user in backend. Please try again.'; isSaving = false; });
                        return;
                      }
                      if (ctx.mounted) {
                        ctx.read<UserBloc>().add(LoadUsers());
  ...
  List<Widget> _permToggles(UserPermissions p, void Function(UserPermissions) onChange) {
    final toggles = [
      ...
      ('⚙️ Settings', 'Can access settings and masters pages', p.canManageMasters, (v) => p.copyWith(canManageMasters: v)),
  ...
  void _confirmDelete(BuildContext ctx, AppUser user) => showDialog(
    ...
    actions: [
      ...
      TextButton(
        onPressed: () async {
          Navigator.pop(ctx);
          final deleted = await ctx.read<UserBloc>().repo.deleteUser(user.username);
        ...
        ElevatedButton(
          onPressed: () async {
            if (ctrl.text.length == 4 && ctrl.text == confirmCtrl.text) {
              final changed = await ctx.read<UserBloc>().repo.changePin(
                user.username,
                ctrl.text,
              );
  ...
  Future<bool> _canAddMoreUsers() async {
    final state = context.read<UserBloc>().state;
    final currentCount = state is UserListLoaded ? state.users.length : 0;
    final maxUsers = await SubscriptionService.instance.getMaxAllowedUsers();
