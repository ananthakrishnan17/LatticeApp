...
class Product extends Equatable {
  ...
  const Product({
    ...
    this.rateType = 'fixed', this.barcode, this.hsnCode, this.sku, this.isActive = true,
  ...
  Product copyWith({
    ...
    String? barcode, String? hsnCode, String? sku, bool? isActive,
    DateTime? createdAt, DateTime? updatedAt,
    String? wholesaleUnit, String? retailUnit,
    double? wholesaleToRetailQty, double? retailPrice,
    String? itemType, String? attributes, String? imageUrl,
  }) => Product(
    ...
    hsnCode: hsnCode ?? this.hsnCode, sku: sku ?? this.sku, isActive: isActive ?? this.isActive,
...
class ProductModel extends Product {
  const ProductModel({
    ...
    super.gstRate, super.gstInclusive, super.rateType, super.barcode, super.hsnCode, super.sku,
    ...
    super.itemType, super.attributes, super.imageUrl,
  });

  factory ProductModel.fromMap(Map<String, dynamic> m) {
    final unit = m['unit'] as String? ?? 'piece';
    ...
    return ProductModel(
      ...
      // DB stores in base units (g / ml). Convert to user-facing units for display.
      stockQuantity: (m['stock_quantity'] as num).toDouble() / factor,
      unit: unit,
      lowStockThreshold: ((m['low_stock_threshold'] as num?)?.toDouble() ?? 5.0) / factor,
      gstRate: (m['gst_rate'] as num?)?.toDouble() ?? 0.0,
      gstInclusive: (m['gst_inclusive'] as int? ?? 1) == 1,
      rateType: m['rate_type'] ?? 'fixed',
      barcode: m['barcode'], hsnCode: m['hsn_code'], sku: m['sku'],
  ...
  Map<String, dynamic> toMap() {
    ...
    return {
      ...
      // Convert user-facing values to base units before storing.
      'stock_quantity': stockQuantity * factor,
      'low_stock_threshold': lowStockThreshold * factor,
      'unit': unit,
      'gst_rate': gstRate, 'gst_inclusive': gstInclusive ? 1 : 0,
      'rate_type': rateType, 'barcode': barcode, 'hsn_code': hsnCode, 'sku': sku,
      ...
      'item_type': itemType, 'attributes': attributes, 'image_url': imageUrl,
    };
  }
}
